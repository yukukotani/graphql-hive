...
export async function createStorage(connection: string, maximumPoolSize: number): Promise<Storage> {
  ...
  const shared = {
    async getUserBySuperTokenId({ superTokensUserId }: { superTokensUserId: string }, connection: Connection) {
      const user = await connection.maybeOne<users>(sql`
        SELECT
          *
        FROM
          public."users"
    ...
    async getOrganization(userId: string, connection: Connection) {
      const org = await connection.maybeOne<organizations>(
    ...
    async createOrganization(
      ...
      const org = await connection.one<organizations>(
        sql`
      ...
      await connection.query<organization_member>(
        sql`
  ...
  const storage: Storage = {
    ...
    async deleteTarget({ organization, project, target }) {
      ...
      await pool.query(sql`DELETE FROM public.registry_versions WHERE target_id = ${target}`);
    ...
    async hasSchema({ target }) {
      return pool.exists(
        sql`
          SELECT 1 FROM public.registry_versions as v WHERE v.target_id = ${target} LIMIT 1
        `
      );
    },
    async getMaybeLatestComposableVersion({ target }) {
      ...
      const version = await pool.maybeOne<
        Pick<registry_versions, 'id' | 'is_composable' | 'created_at' | 'action_id' | 'base_schema'>
      >(
        sql`
          SELECT 
            id,
    ...
    async getLatestSchemas({ organization, project, target }) {
      const latest = await pool.maybeOne<Pick<registry_versions, 'id'>>(sql`
        SELECT v.id FROM public.registry_versions as v
    ...
    async getSchemasOfVersion({ version, includeMetadata = false }) {
      const results = await pool.many<registry_actions | Omit<registry_actions, 'metadata'>>(
        sql`
          SELECT
            c.id,
            c.commit,
            ...
            c.action
          FROM public.registry_version_action AS vc
          LEFT JOIN public.registry_actions AS c ON c.id = vc.action_id
          WHERE
    ...
    async getSchemasOfPreviousVersion({ version, target }) {
      const results = await pool.query<registry_actions>(
        sql`
          SELECT c.*, vc.url FROM public.registry_version_action as vc
          LEFT JOIN public.registry_actions as c ON (c.id = vc.action_id)
          WHERE vc.version_id = (
            SELECT v.id FROM public.registry_versions as v WHERE v.created_at < (
              SELECT vi.created_at FROM public.registry_versions as vi WHERE vi.id = ${version}
    ...
    async updateSchemaUrlOfVersion({ version, commit, url }) {
      await pool.query(
        sql`
          UPDATE public.registry_version_action
          SET url = ${url ?? null}
          WHERE version_id = ${version} AND action_id = ${commit}
        `
      );
    },
    ...
    async getVersion({ target, version }) {
      const result = await pool.one<
        Pick<registry_versions, 'id' | 'is_composable' | 'created_at' | 'action_id' | 'base_schema'> &
          Pick<registry_actions, 'author' | 'service_name'>
      >(sql`
        SELECT
          v.id,
          v.is_composable,
          ...
          c.author,
          c.service_name
        FROM public.registry_versions as v 
        LEFT JOIN public.registry_actions as c ON (c.id = v.action_id)
    ...
    async getVersions({ target, after, limit }) {
      const query = sql`
        SELECT v.* FROM public.registry_versions as v 
        WHERE v.target_id = ${target} AND v.created_at < ${
        after ? sql`(SELECT va.created_at FROM public.registry_versions as va WHERE va.id = ${after})` : sql`NOW()`
      }
        ORDER BY v.created_at DESC
        LIMIT ${limit + 1}
      `;
    ...
    deleteSchema({ serviceName, isComposable, baseSchema, author, commit: userCommit, project, target }) {
      return pool.transaction(async t => {
        const currentCommits = await t.query<
          Pick<registry_version_action, 'action_id' | 'version_id'> & Pick<registry_actions, 'action' | 'service_name'>
        >(sql`
            SELECT vc.action_id, vc.version_id, c.action, c.service_name
            FROM public.registry_version_action as vc
            INNER JOIN public.registry_actions as c ON (c.id = vc.action_id)
            WHERE vc.version_id = (
              SELECT id FROM public.registry_versions
              WHERE target_id = ${target}
              ORDER BY created_at DESC
              LIMIT 1
            )
        ...
        const commit = await t.one<registry_actions>(sql`
        ...
        const newVersion = await t.one<Pick<registry_versions, 'id' | 'created_at'>>(sql`
    ...
    async updateVersionStatus({ version, valid }) {
      return transformRegistryVersion(
        await pool.one<registry_versions>(sql`
    ...
    getSchema: batch(async selectors => {
      const rows = await pool.many<registry_actions>(
        sql`
            SELECT c.*
            FROM public.registry_actions as c
